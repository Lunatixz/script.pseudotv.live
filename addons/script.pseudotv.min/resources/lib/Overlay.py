...
class MyPlayer(xbmc.Player):
    ...
    def log(self, msg, level = xbmc.LOGNOTICE):
        logGlob('Overlay: ' + msg, level)
    
    def logDebug(self, msg):
        logGlob('Overlay: ' + msg)
    
    def logError(self, msg):
        logGlob('Overlay: ' + msg, xbmc.LOGERROR)
...
class TVOverlay(xbmcgui.WindowXMLDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if FileAccess.exists(os.path.join(XBMC_SKIN_LOC, 'custom_script.pseudotv.min_9506.xml')):
    ...
    def log(self, msg, level = xbmc.LOGNOTICE):
        logGlob('TVOverlay: ' + msg, level)
    
    def logDebug(self, msg):
        logGlob('TVOverlay: ' + msg)
    
    def logError(self, msg):
        logGlob('TVOverlay: ' + msg, xbmc.LOGERROR)
    ...
    def Backup(self, org, bak):
        ...
        if DEBUG == 'true':
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "Backup Complete", 1000, THUMB) )
    ...
    def Restore(self, bak, org):
        ...
        xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "Restore Complete, Restarting...", 1000, THUMB) )

        
    # override the doModal function so we can setup everything first
    def onInit(self):
        self.log('onInit')
        self.logDebug('PTVL Version = ' + ADDON_VERSION)
        ...
        json_query = ('{"jsonrpc": "2.0", "method": "JSONRPC.NotifyAll", "params": {"sender":"PTVL","message":"PseudoTV_Min: Started"}, "id": 1}')
        self.channelList.sendJSON(json_query)
            ...
            if REAL_SETTINGS.getSetting("reserveChannels") == "false":
                ...
                if settingsFile_flesize > 100:
                    ...
                    if FileAccess.exists(atsettingsFile):
                        ...
                        f.close()
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "Initializing Autotuning...", 4000, THUMB) )

        if FileAccess.exists(GEN_CHAN_LOC) == False:
            try:
                FileAccess.makedirs(GEN_CHAN_LOC)
            except:
                self.Fatal('Unable to create the cache directory')
                return
        ...
        if FileAccess.exists(MADE_CHAN_LOC) == False:
            try:
                ...
                self.Fatal('Unable to create the storage directory')
                return
        ...
        updateDialog.create("PseudoTV Min", "Initializing")
        ...
        updateDialog.update(95, "Initializing", "PseudoTV Min")
        ...
        try:
            self.myEPG = EPGWindow("script.pseudotv.min.EPG.xml", ADDON_PATH, Skin_Select)
        except:
            pass
        ...
        if self.maxChannels == 0 and REAL_SETTINGS.getSetting("Autotune") == "false":
            ...
            if dlg.yesno("No Channels Configured", "Would you like PseudoTV Min to AutoTune Channels?"):
                ...
                if autoTune:
                    xbmc.executebuiltin('XBMC.AlarmClock( Restarting PseudoTV Min, XBMC.RunScript(' + ADDON_PATH + '/default.py),0.5,true)')
                    self.end()
                    return
            else:
                REAL_SETTINGS.setSetting("Autotune","false")
                REAL_SETTINGS.setSetting("Warning1","false")
                self.Fatal('Unable to find any channels. \nPlease go to the Addon Settings to configure PseudoTV Min.')
            ...
            if self.maxChannels == 0:
                self.Fatal('Unable to find any channels. Please configure the addon.')
                REAL_SETTINGS.openSettings()
                self.end()
                return
        ...
        if found == False:
            self.Fatal("Unable to populate channels. Please verify that you", "have scraped media in your library and that you have", "properly configured channels.")
            return
        ...
        try:
            if self.forceReset == False:
                StartUpChannel = int(REAL_SETTINGS.getSetting('StartUpChannel'))
                
                if StartUpChannel == 1:
                    self.currentChannel = self.fixChannel(int(REAL_SETTINGS.getSetting("PreferredChannel")))
                elif StartUpChannel == 2:
                    sTrace = "random channel selecting between %s - %s" %(str(1), str(self.maxChannels)) #todo: selectable range?
                    self.logDebug(sTrace)
                    
                    iRandChan = random.randint(1, self.maxChannels)
                    self.logDebug("random channel selected:" + str(iRandChan))
                    
                    self.currentChannel = self.fixChannel(iRandChan)
            else:
                self.currentChannel = self.fixChannel(1)
        except:
            self.Fatal("Attempting to fix channel for 2nd time")
            self.currentChannel = self.fixChannel(1)

        if REAL_SETTINGS.getSetting('INTRO_PLAYED') != 'true':
            REAL_SETTINGS.setSetting("INTRO_PLAYED","true")
        
        self.resetChannelTimes() #todo: investigate
        ...
        if REAL_SETTINGS.getSetting('StartupMessage') == "false":
            if self.channelList.autoplaynextitem == True:
                self.message('Its recommend you DISABLE Kodi Video Playback Setting "Play the next video Automatically"')
    ...
    def Settop(self):
        ...
        if NOTIFY == 'true':
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "Settop Update Complete", 1000, THUMB) )
    ...
    def readConfig(self):
        ...
        self.smallChannelBug = REAL_SETTINGS.getSetting("SmallChannelBug") == "true"
        self.log('Small channel bug - ' + str(self.smallChannelBug))
    ...
    def Fatal(self, line1, line2 = '', line3 = ''):
    ...
    def message(self, data):
        ...
        dlg = xbmcgui.Dialog()
        dlg.ok('PseudoTV Min Announcement:', data)
        del dlg
    ...
    def setOnNow(self):
        ...
        if DEBUG == 'true':
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "DEBUGGING: setOnNow finished", 1000, THUMB) )
    ...
    def setChannel(self, channel):
        ...
        json_query = ('{"jsonrpc": "2.0", "method": "JSONRPC.NotifyAll", "params": {"sender":"PTVL","message":"PseudoTV_Min: Channel Name - %s"}, "id": 1}' % (chname))
        self.channelList.sendJSON(json_query)
        ...
        if self.Player.stopped:
            self.logError('setChannel player already stopped');
            return

        if channel < 1 or channel > self.maxChannels:
            self.logError('setChannel invalid channel ' + str(channel))
            return

        if self.channels[channel - 1].isValid == False:
            self.logError('setChannel channel not valid ' + str(channel))
            return
        ...
        if xbmc.PlayList(xbmc.PLAYLIST_MUSIC).load(self.channels[channel - 1].fileName) == False:
            self.logError("Error loading playlist")
        ...
        if self.channels[self.currentChannel - 1].isPaused:
            ...
            try:
                ...
                self.logError('Exception during seek on paused channel')
                pass
            self.Paused()
        else:  
            #if plugchk not in BYPASS_SEEK:
            ...
            if (mediapath[-4:].lower() == 'strm' or mediapath[0:6].lower() == 'plugin'):
                ...
                try:
                    ...
                    try:
                        ...
                        self.logError('Exception during seek')
                        pass
    ...
    def SmartSeek(self, mediapath, seektime1, seektime2, overtime):
        ...
        if seektime == 0 and DEBUG == 'true':
            ...
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "DEBUGGING: Overriding Seektime", 1000, THUMB) )
        return seektime    
    ...
    def InvalidateChannel(self, channel):
        ...
        if self.invalidatedChannelCount > 3:
            self.Fatal("Exceeded 3 invalidated channels. Exiting.")
            return
        ...
        if remaining == 0:
            self.Fatal("No channels available. Exiting.")
            return
    ...
    def waitForVideoPaused(self):
        ...
        while sleeptime < TIMEOUT:
            ...
            self.logError('Timeout waiting for pause')
            return False
    ...
    def SetMediaInfo(self, chtype, chname, mediapath, position):
        ...
        try:
            ...
            self.logError('setShowInfo, Label 512 not found')
            pass  
        
        iDynArt1 = 508
        #Dynamic Art1
        try:
            self.getControl(iDynArt1).setVisible(False)
            type1EXT = REAL_SETTINGS.getSetting('type1EXT_Overlay')
            self.setArtwork1(type, chtype, chname, id, dbid, mpath, type1EXT)
        except:
            self.logError('setShowInfo, Label %s not found' %iDynArt1)
            pass
           
        iDynArt2 = 510
        #Dynamic Art2
        try:
            self.getControl(iDynArt2).setVisible(False)
            type2EXT = REAL_SETTINGS.getSetting('type2EXT_Overlay')
            self.setArtwork2(type, chtype, chname, id, dbid, mpath, type2EXT)
        except:
            self.logError('setShowInfo, Label %s not found' %iDynArt2)
            pass
    ...
    def setArtwork1(self, type, chtype, chname, id, dbid, mpath, type1EXT):
        ...
        try:
            ...
            self.getControl(iDynArt1).setImage(setImage1)
            self.getControl(iDynArt1).setVisible(True)
        except:
            self.getControl(iDynArt1).setVisible(False)
            self.logError('setArtwork1, Failed!')
            pass  
    ...
    def setArtwork2(self, type, chtype, chname, id, dbid, mpath, type2EXT):
        ...
        try: 
            ...
            self.getControl(iDynArt2).setImage(setImage2)
            self.getControl(iDynArt2).setVisible(True)
        except:
            self.getControl(iDynArt2).setVisible(False)
            self.logError('setArtwork2, Failed!')
            pass
    ...
    def showChannelLabel(self, channel):
        ...
        try:
            if self.showChannelBug == True:
                fpChanBug = self.Artdownloader.FindBug(chtype, chname, mediapath)
                if self.smallChannelBug == True:
                    self.getControl(104).setImage(fpChanBug)
                else:
                    self.getControl(103).setImage(fpChanBug)
            else:
                self.getControl(103).setImage('NA.png')
        except:
            self.logError('Failed to render ChanBug')
    ...
    def OnDemandAction(self, type='OnDemand'):
        ...
        if type == 'Search':
            ...
            self.Paused()
            self.hideInfo()
            extTypes = ['.avi', '.flv', '.mkv', '.mp4', '.ts']
            self.Browse = dlg.browse(1,'OnDemand', 'video', '.avi|.flv|.mkv|.mp4|.ts', True, True, 'special://videoplaylists')
    ...
    def hideMenuAlt(self):
        ...
        try:
            ...
            self.logError('hideMenuAlt failed')
    ...
    def ShowMenuAlt(self, timer):
        ...
        try:
            ...
            self.logError('ShowMenuAlt failed')
    ...
    def pauseMenuAlt(self):
        ...
        try:
            ...
            self.logError('pauseMenuAlt failed')
    ...
    def showMenu(self, timer):
        ...
        try:
            ...
            self.getControl(1001).setLabel('Random Channel')
    ...
    def pauseMenu(self):
        ...
        try:
            ...
            self.logError('pauseMenu failed')
    ...
    def SleepButton(self):
        ...
        self.startSleepTimer()
        xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", SMSG, 1000, THUMB) )    
    ...
    def onClick(self, controlId):
        ...
        if controlId == 997:
            ...
            if self.showingMenu:
                self.log("Random Channel")
                sTrace = "random channel selecting between %s - %s" %(str(1), str(self.maxChannels))
                self.logDebug(sTrace)
            ...
            if self.showingMenu:
                self.log("Open Numpad (Coming Soon)") #todo:
            ...
            if self.showingMenu:
                self.log("Settings")
                self.pauseMenu()
                xbmcaddon.Addon(id='script.pseudotv.min').openSettings()
                self.showMenu(self.InfTimer)
            ...
            if self.showingMenu:
                self.log("Exit")
                self.pauseMenu()
                if dlg.yesno("Exit?", "Are you sure you want to exit PseudoTV Min?"):
                    self.hideMenu()
                    self.end()
                else:
                    self.showMenu(self.InfTimer)
        else:
            logError('Unknown controlId in onClick: ' %controlId)
    ...
    def onAction(self, act):
        action = act.getId()
        self.logDebug('onAction ' + str(action))
        self.onActionIdleTimer = 0
        
        try:
            chtype = int(ADDON_SETTINGS.getSetting('Channel_' + str(self.currentChannel) + '_type'))
        except:
            self.logError('Failed to get chtype on first try, retrying')
            chtype = int(ADDON_SETTINGS.getSetting('Channel_' + str(self.currentChannel) + '_type'))
        ...
        if action == ACTION_SELECT_ITEM:
            self.logDebug('ACTION_SELECT_ITEM')
            ...
            self.logDebug('ACTION_MOVE_UP || ACTION_PAGEUP')
            if self.showingMenuAlt:
                self.setOnNowArt()
            elif not self.showingMenu:
                self.channelUp()
            ...
            self.logDebug('ACTION_MOVE_DOWN || ACTION_PAGEDOWN')
            if self.showingMenuAlt:
                self.setOnNowArt()
            elif not self.showingMenu:
                self.channelDown()
            ...
            self.logDebug('ACTION_NUMBER_[%s]' %(action-58))
            ...
            if not self.showingEPG:
                if not self.showingMenu:
                    if not self.showingMenuAlt:
                        self.logDebug('Show EPG')
                        self.hideInfo()
                        self.hidePOP()
                        self.ShowEPG()
            else:
                self.logDebug('Hide EPG') #shouldn't get here - handled by EPG now
                #xbmc.executebuiltin('XBMC.Action(Escape)')
        
        elif action == ACTION_CLICK_RIGHT:
            self.logDebug('ACTION_CLICK_RIGHT')
        
        elif action == ACTION_MOUSE_MOVE:
            self.logDebug('ACTION_MOUSE_MOVE')
            
        elif action == ACTION_GESTURE_PAN:
            self.logDebug('ACTION_GESTURE_PAN')
            
        elif action == ACTION_GESTURE_BEGIN:
            self.logDebug('ACTION_GESTURE_BEGIN')
            
        elif action == ACTION_GESTURE_END:
            self.logDebug('ACTION_GESTURE_END')
        
        elif action == ACTION_TRIGGER_OSD:
            self.logDebug('ACTION_TRIGGER_OSD')
            # xbmc.executebuiltin("ActivateWindow(12901)")
        
        else:
            if DEBUG == 'true':
                self.logError('Unknown onAction ID: %s' %action)
        
        self.actionSemaphore.release()
        self.log('onAction return: [%s]' %action)
...
#        self.sleepTimer = threading.Timer(self.sleepTimeValue, self.sleepAction)
    ...
    def PlayerTimeout(self, start_time):
        ...
        if get_time == start_time: 
                ...
                if DEBUG == 'true':
                    xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "DEBUGGING: PlayerTimeout Stopped!", 1000, THUMB) )
    ...
    def playerTimerAction(self):
        ...
        if self.notPlayingCount > 5:
            ...
            if DEBUG == 'true' and self.notPlayingCount > 1:
                xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "notPlayingCount " + str(self.notPlayingCount), 1000, THUMB) )
    ...
    def SetAutoJump(self, time, cleanTime, title, channel):
        ...
        xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min","Reminder Set for " + str(cleanTime), 4000, THUMB) )
    
    
    def AutoJump(self, title, channel):
        self.log('AutoJump') 
        msg = title + ' starts in 1m'
        if dlg.yesno("PseudoTV Min", msg, "Would you like to switch to channel " + str(channel) + ' ?'):
    ...
    def EPGtypeToggle(self):
        ...
        if ColorType == '0':
            ...
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "EPG Color by Genre ", 1000, THUMB) )
        elif ColorType == '1':
            REAL_SETTINGS.setSetting("EPGcolor_enabled", "2")
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "EPG Color by Chtype", 1000, THUMB) )
        elif ColorType == '2':
            REAL_SETTINGS.setSetting("EPGcolor_enabled", "3")
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "EPG Color by Rating", 1000, THUMB) )
        elif ColorType == '3':
            REAL_SETTINGS.setSetting("EPGcolor_enabled", "0")
            xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "EPG Color Disabled", 1000, THUMB) )
    ...
    def ArtService(self):
        ...
        if REAL_SETTINGS.getSetting("ArtService_Running") == "false":
            ...
            if NOTIFY == 'true':
                xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", "Artwork Spooler Started", 4000, THUMB) )
            ...
            if NOTIFY == 'true':
                xbmc.executebuiltin("Notification( %s, %s, %d, %s)" % ("PseudoTV Min", MSSG, 1000, THUMB) ) 
    ...
    def end(self):
        ...
        updateDialog = xbmcgui.DialogProgress()
        updateDialog.create("PseudoTV Min", "Exiting")
        ...
        if self.channelThread.isAlive():
            ...
            if self.channelThread.isAlive():
                self.logError("Problem joining channel thread")
        ...
        if self.isMaster:
            ...
            try:#Startup Channel
                StartUpChannel = int(REAL_SETTINGS.getSetting('StartUpChannel'))
                if StartUpChannel == 0:
                    REAL_SETTINGS.setSetting('PreferredChannel', str(self.currentChannel))
            except:
                pass
        ...
        json_query = ('{"jsonrpc": "2.0", "method": "JSONRPC.NotifyAll", "params": {"sender":"PTVL","message":"PseudoTV_Min: Stopped"}, "id": 1}')
        self.channelList.sendJSON(json_query)
        REAL_SETTINGS.setSetting('Normal_Shutdown', "true")
        updateDialog.close()
        self.background.setVisible(False)
