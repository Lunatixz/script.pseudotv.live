...
class TMDB(object):
    ...
    def getMovie_NEW(self, movieName, year):
        ...
        try:
            ...
            if response['total_results'] > 0:
                ...
                logGlob('No matches found for %s' % movieName)
    ...
    def get_image_list(self, media_id):
        ...
        if data == "Empty" or not data:
            ...
            try:
                ...
                logGlob( 'Problem report: %s' %str( e ), xbmc.LOGERROR )
            # Get thumbs
            try:
                for item in data['backdrops']:
                    if int(item.get('vote_count')) >= 1:
    ...
    def _search_movie(medianame,year=''):
        ...
        logGlob('TMDB API search criteria: Title[''%s''] | Year[''%s'']' % (medianame,year) )
        illegal_char = ' -<>:"/\|?*%'
        ...
        tmdb_id = ''
        logGlob('TMDB API search:   %s ' % search_url)
        try:
            ...
            logGlob( str( e ), xbmc.LOGERROR )
        if tmdb_id == '':
            logGlob('TMDB API search found no ID')
        else:
            logGlob('TMDB API search found ID: %s' %tmdb_id)
        return tmdb_id


    # Retrieve JSON data from cache function
    def get_data(self, url, data_type ='json'):
        logGlob('Downloader: get_data - Cache')
        if CACHE_ON:
            try:
        ...
        logGlob('Downloader: result = ' + str(result))
        return result

    # Retrieve JSON data from site
    def get_data_new(self, url, data_type):
        logGlob('Downloader: get_data_new - Cache expired. Retrieving new data')
        data = []
        try:
