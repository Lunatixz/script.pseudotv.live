...
LOG_PREFIX = 'FileAccess: '
LOG_PREFIX2 = 'FileLock: '

def logFA(msg, level = xbmc.LOGDEBUG):
    if DEBUG_N == 'true':
        level = xbmc.LOGNOTICE
    xbmc.log(ADDON_ID + '-F-' + msg, level)


class FileAccess:
    @staticmethod
    def log(msg, level = xbmc.LOGNOTICE):
        logFA(LOG_PREFIX + msg, level)
    
    @staticmethod
    def logDebug(msg):
        logFA(LOG_PREFIX + msg)
            
    @staticmethod
    def logError(msg):
        logFA(LOG_PREFIX + msg, xbmc.LOGERROR)


    @staticmethod
    def open(filename, mode, encoding = "utf-8"):
        fle = 0
        FileAccess.logDebug("opening: " + filename)
        
        try:
            FileAccess.logDebug("opening attempt 1")
            return VFSFile(filename, mode)
        except UnicodeDecodeError:
            FileAccess.logDebug("opening attempt 2")
    ...
    def copy(orgfilename, newfilename):
        FileAccess.logDebug("copying " + orgfilename + " to " + newfilename)
    ...
    @staticmethod
    def exists(filename):
        FileAccess.logDebug("exists: " + filename)
    ...
    def rename(path, newpath):
        FileAccess.logDebug("rename " + path + " to " + newpath)
        ...
        if path[0:6].lower() == 'smb://' or newpath[0:6].lower() == 'smb://':
            if os.name.lower() == 'nt':
                FileAccess.logDebug("Modifying name")
        ...
        try:
            ...
            FileAccess.logDebug("shutil.move")
            return True
        except Exception,e:
            pass

        FileAccess.logDebug("OSError")
        raise OSError()
    ...
    def _makedirs(path):
        ...
        FileAccess.logDebug('_mkdirs: ' + path)
        ...
        if success == False:
            ...
            if FileAccess._makedirs(os.path.dirname(path)):
                FileAccess.FileAccess.logDebug('_mkdirs2: ' + path)
...
class VFSFile:
    def __init__(self, filename, mode):
        ...
        if self.currentFile == None:
            logFA("VFSFile: Couldnt open " + filename, xbmc.LOGERROR)
        else:
            logFA("VFSFile: Opening " + filename)
...
class FileLock:
    ...
    def log(self, msg, level = xbmc.LOGNOTICE):
        logFA(LOG_PREFIX2 + msg, level)
    
    def logDebug(self, msg):
        logFA(LOG_PREFIX2 + msg)
    
    def logError(self, msg):
        logFA(LOG_PREFIX2 + msg, xbmc.LOGERROR)
