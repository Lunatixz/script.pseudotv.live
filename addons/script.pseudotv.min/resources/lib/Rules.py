...
class RulesList:
    def __init__(self):
        self.ruleList = [BaseRule(), ScheduleChannelRule(), HandleChannelLogo(), NoShowRule(), DontAddChannel(), EvenShowsRule(), ForceRandom(), ForceRealTime(), ForceResume(), Handle3D(), HandlePOP(), InterleaveChannel(), OnlyUnWatchedRule(), OnlyWatchedRule(), AlwaysPause(), PlayShowInOrder(), RenameRule(), SetResetTime()]
...
class BaseRule:
    ...
    def log(self, msg, level = xbmc.LOGNOTICE):
        logGlob('Rules: ' + self.getTitle() + ": " + msg, level)
    
    def logDebug(self, msg):
        logGlob('Rules: ' + self.getTitle() + ": " + msg)
    
    def logError(self, msg):
        logGlob('Rules: ' + self.getTitle() + ": " + msg, xbmc.LOGERROR)
