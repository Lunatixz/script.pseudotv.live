...
class ChannelListThread(threading.Thread):
    ...
    def log(self, msg, level = xbmc.LOGNOTICE):
        logGlob('ChannelListThread: ' + msg, level)
    
    def logDebug(self, msg):
        logGlob('ChannelListThread: ' + msg)
    
    def logError(self, msg):
        logGlob('ChannelListThread: ' + msg, xbmc.LOGERROR)
    ...
    def run(self):
        ...
        if self.fullUpdating and self.myOverlay.isMaster:
            if validchannels < self.chanlist.enteredChannelCount:
                title = 'PseudoTV Min, Background Loading...'
                    ...
                    try:
                        if self.chanlist.setupChannel(i + 1, True, True, False) == True:
                            ...
                            if self.myOverlay.channels[i].isValid == True:
                                title = "PseudoTV Min, Channel " + str(i + 1) + " Added"
                                xbmc.executebuiltin('XBMC.Notification(%s, %s, %s)' % (title, 4000, THUMB))
                                
                    except Exception,e:
                        self.logError("Unknown Channel Creation Exception")
                    ...
                    if self.myOverlay.isMaster:
                        if curtotal > 0:
                            ...
                            try:
                                ...
                                self.logError("Unknown Channel Appending Exception")
                                self.logError(traceback.format_exc())
                                return
                            ...
                            try:
                                ...
                                self.logError("Unknown Channel Modification Exception")
                                self.logError(traceback.format_exc())
                                return
                    else:
                        try:
                            ...
                            self.logError("Unknown Channel Loading Exception")
                            self.logError(traceback.format_exc())
                            return
