...
class EPGWindow(xbmcgui.WindowXMLDialog):
    ...
    def log(self, msg, level = xbmc.LOGNOTICE):
        logGlob('EPGWindow: ' + msg, level)
    
    def logDebug(self, msg):
        logGlob('EPGWindow: ' + msg)
    
    def logError(self, msg):
        logGlob('EPGWindow: ' + msg, xbmc.LOGERROR)
    ...
    def onInit(self):
        ...
        try:
            ...
            self.logError("Unknown EPG Initialization Exception")
            self.logError(traceback.format_exc())
            try:
                self.close()
            except:
                self.logError("Error closing")
    ...
    def setChannelButtons(self, starttime, curchannel, singlerow = -1):
       ...
       # This is done twice to guarantee we go back 2 channels.  If the previous 2 channels
        ...
        try:
            ...
            self.logError('self.addControls(myadds) in use')
            pass
    ...
    def setButtons(self, starttime, curchannel, row):
        ...
        try:
            ...
            self.logError("Exception in setButtons")
            self.logError(traceback.format_exc())
    ...
    def onAction(self, act):
        ...
        try:
            if action in ACTION_PREVIOUS_MENU:
                self.logDebug('ACTION_PREVIOUS_MENU')
                self.closeEPG()           
                if self.showingInfo:
                    self.infoOffset = 0
                    self.infoOffsetV = 0
            ...
            elif action == ACTION_CLICK_LEFT: #if not selecting, this should be fine
            ...
            self.logError("Unknown EPG Exception")
            self.logError(traceback.format_exc())
    ...
    def setShowInfo(self):
        ...
        if self.MyOverlayWindow.VideoWindow == True:
            ...
            try:
                ...
                self.getControl(524).setLabel("control 524 label") # self.MyOverlayWindow.PVRtitle
            except:
                pass
        else:
    ...
    def SetMediaInfo(self, chtype, chname, mediapath, newchan, plpos):
        ...
        try:
            ...
            self.logError('setShowInfo, Label 508 not found')
            pass
        ...
        try:
            ...
            self.logError('setShowInfo, Label 510 not found')
            pass
    ...
    def setArtwork1(self, type, chtype, chname, id, dbid, mpath, type1EXT):
        ...
        try:
            ...
            self.getControl(508).setVisible(False)
            self.logError('setArtwork1, Failed!' + str(e))
            pass  
    ...
    def setArtwork2(self, type, chtype, chname, id, dbid, mpath, type2EXT):
        ...
        try: 
            ...
            self.getControl(510).setVisible(False)
            self.logError('setArtwork2, Failed!' + str(e))
            pass
    ...
    def selectShow(self):
        ...
        try:
            ...
            if plpos == -1:
                self.logError('Unable to find the proper playlist to set from EPG')
                return
            ...
            if pos != plpos:
                if chtype == 8:
                    ...
                    if dlg.yesno("PseudoTV Min", "Would you like to set a reminder for [B]", str(self.PVRtitle) + '[/B] on channel [B]' + str(self.PTVChanNum), '[/B]at [B]'+ str(Notify_Time) + '[/B] ?'):
                    ...
                    if REAL_SETTINGS.getSetting("SelectAction") == "1":
                        ...
                        if dlg.yesno("PseudoTV Min", "Would you like to watch [B]", str(self.PVRtitle) + '[/B] Now or set a reminder for channel [B]' + str(self.PTVChanNum), '[/B]at [B]'+ str(Notify_Time) + '[/B] ?'):
    ...
    def setReminder(self, tmpDate, cleanDate, title, channel):
        ...
        if jump == False:
            xbmc.executebuiltin('XBMC.AlarmClock(PseudoTV Min, XBMC.Notification("PseudoTV Min",'+ msg +',4000,'+ THUMB +'),'+ str(reminder_time) + ',false)')
        else:
